...
# support for DaphneDSL
from multimetric.daphne_lexer import DaphneDSLLexer
import os
...
def file_process(_file, _args, _importer):
    ...
    # check if DaphneDSL file
    if os.path.splitext(_file)[1].lower() == ".daph":
        _lexer = DaphneDSLLexer()
    else:
        try:
